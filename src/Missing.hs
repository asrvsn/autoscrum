...
mean :: Floating a => [a] -> a
mean x = fst $ foldl' (\(!m, !n) y -> (m+(y-m)/(n+1),n+1)) (0,0) x
...
var xs = (var' 0 0 0 xs) / (fromIntegral $ length xs - 1)
    where
      var' _ _ s [] = s
      var' m n s (y:ys) = var' nm (n + 1) (s + delta * (y - nm)) ys
         where
           delta = y - m
...
trace' :: (Show b) => String -> b -> b
trace' a b = trace (a <> show b) b

lookup :: (HasCallStack, Show k, Eq k, Hashable k) => HashMap k v -> k -> v
